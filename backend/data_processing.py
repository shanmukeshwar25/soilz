...
DATA_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'ProcessedData.csv')
...
def load_and_clean_data(csv_path: str) -> pd.DataFrame:
    ...
    # Convert g/ha → kg/ha and normalize unit
    df.loc[df['UnitS'] == 'g/ha', 'ValueS'] = df.loc[df['UnitS'] == 'g/ha', 'ValueS'] / 1000
    df['UnitS'] = 'kg/ha'
    ...
    agg_df = df.groupby(
        ['Crop', 'SoilType', 'CreatedDate', 'BatchId', 'CropStartDate', 'CropEndDate', 'Measure', 'UnitS']
    ).agg({'ValueS': 'mean'}).reset_index()
...
def get_summary_stats(crop: str, soil: str):
    ...
    summary = []
    for (measure, unit), group in sub_df.groupby(['Measure', 'UnitS']):
        group_sorted = group.sort_values('CreatedDate')
        summary.append({
            "measure": measure,
            "latest":  float(group_sorted.iloc[-1]['ValueS']),
            "average": float(group['ValueS'].mean()),
            "min":     float(group['ValueS'].min()),
            "max":     float(group['ValueS'].max()),
            "unit":    unit
        })
